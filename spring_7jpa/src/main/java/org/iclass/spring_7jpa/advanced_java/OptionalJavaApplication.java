...
public class OptionalJavaApplication {
	public static void main(String[] args) {
		Optional<String> noneEmpty = Optional.of("HI STREAM");	// null이 아닌 값을 저장하는 컨테이너 Optional 생성
		...
		// 3) isEmpty() 일 떄 처리 메소드
		String result = nullable.orElse("기본값");		// orElse 메소드 인자는 값(T 타입)
		System.out.println("nullable.erElse(\"기본값\") 리턴: ");
		...
		// result = nullable.orElseThrow();	// 값이 없으면 예외 발생(No value present)

		// 4) if(noneEmpty.isPresn)
		final int[] legth = new int[1];
		noneEmpty.ifPresent(s -> {
			legth[1] = s.length();	// 여기서는 final 변수만 사용 가능. 일반 final int 는 ❌, 배열 사용
			final int test = s.length();	// 사용 못함
